...
public class NonogramUI {
    ...
    */
    ...
    do {
        int x = 0;
        do {
            savedString += puzzle.getState(y, x);
            x++;
        } while (x < puzzle.getNumRows());
        y++;
    } while (y < puzzle.getNumCols());
    Path path = Paths.get("savedFiles/savedNonogram.txt");
    try {
        if (Files.notExists(path)) {
            Files.createDirectories(path.getParent());
            Files.createFile(path);
        }
        Files.write(path, savedString.getBytes());
    } catch (IOException e) {
        e.printStackTrace();
    }
  }
    ...
    public boolean load() {
        ...
        try {
            if (Files.notExists(path)) {
                System.out.println("Saved Puzzle not found");
            } else {
                String board = new String(Files.readAllBytes(path));
                if((puzzle.getNumCols() * puzzle.getNumRows()) == board.length()){
                    puzzle.setStatesByString(board);
                    return true;
                }else{
                    System.out.println("Puzzle was not saved correcly in the file");
                    return false;
                }
            }
        } catch (IOException e) {
    ...
    */
    public static void help(JTextArea txtarea) {
        txtarea.append("Nonogram is a puzzle where you must colour in/fill in the grid according to the patterns");
